...
export const getRequestHandler = async (ctx: Context, req: express.Request<{ request_id?: string }>, res: express.Response) => {
  let username = '';
  if (req.headers.authorization) {
    username = (getTokenPayload(req.headers.authorization.split(' ')[1], process.env.SECRET_TOKEN ?? '') as JwtPayload).username;
  }
